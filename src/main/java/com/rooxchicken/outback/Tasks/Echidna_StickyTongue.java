...
public class Echidna_StickyTongue extends Task
{
    ...
    public Echidna_StickyTongue(Outback _plugin, Player _player)
    {
        ...
        for(Object o : Library.getNearbyEntities(player.getLocation(), 20))
        {
            if(o instanceof LivingEntity && o != player)
            {
                ...
                entity.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 200, 2));
                entity.addPotionEffect(new PotionEffect(PotionEffectType.POISON, 200, 0));
                entity.addPotionEffect(new PotionEffect(PotionEffectType.NAUSEA, 200, 0));
    ...
    public void run()
    {
        ...
        for(int i = 0; i < 10; i++)
        {
            player.getWorld().spawnParticle(Particle.DUST, player.getLocation().clone().add(0,1,0).add(target.getLocation().clone().add(0,1,0).subtract(player.getLocation()).multiply(i/10.0)), 2, 0, 0, 0, new Particle.DustOptions(Color.fromRGB(0x882222), 0.6f));
        }
